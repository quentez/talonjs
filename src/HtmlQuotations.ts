...
import { CheckpointPrefix, CheckpointSuffix, NodeLimit, QuoteIds, NodeType } from './Constants';
...
export function addCheckpoint(document: Document, element: Node, count: number = 0, level: number = 0): number {
  ...
  if (element.firstChild && element.firstChild.nodeType === NodeType.TEXT_NODE)
  ...
  if (element.nextSibling && element.nextSibling.nodeType === NodeType.TEXT_NODE)
